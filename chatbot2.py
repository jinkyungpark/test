...
if prompt := st.chat_input("문의하실 내용을 입력해 주세요"):
    ...
    class StreamHandler(AssistantEventHandler):
        ...
        def on_text_created(self, text):
            '''Assistant가 텍스트 응답 생성을 시작할 때 호출'''
            self.text = ""

        @override
        def on_text_delta(self, delta, snapshot):
            '''Assistant가 응답 중간 중간 부분 텍스트(delta) 를 보내올 때마다 호출'''
            self.text += delta.value
            output_area.markdown(self.text + "▌")  # 타자기 효과

        @override
        def on_text_done(self, text):
            """텍스트 생성이 완전히 종료되었을 때 호출"""
            output_area.markdown(self.text)  # 최종 마크다운 출력
